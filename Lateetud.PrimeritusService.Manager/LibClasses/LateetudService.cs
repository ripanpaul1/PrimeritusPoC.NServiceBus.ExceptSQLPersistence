...
namespace Lateetud.PrimeritusService.Manager.LibClasses
{
    ...
    {
        public VMExcel ExcelToXml(HttpPostedFileBase file, string SheetName, string rootPath, string DirExcel, string DirXml, string RequestType)
        {
            ...
            if (file != null && file.ContentLength > 0)
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        List<string> NotesTag = new List<string>(new string[] { "<Notes>", "</Notes>", "<Note>", "</Note>" });
                        ...
                        for (int row = 1; row < dtexcel.Rows.Count; row++)
                        {
                            ...
                            if (RequestType == "1")
                            {
                                ...
                                // create the file
                                vMExcel.ConvertedXmlFileName = DateTime.Now.ToString("yyyyMMdd") + "-" + DateTime.Now.ToString("HHmmssfff") + "-" + new RandomGenerator().RandomGuid() + "-New-Assignment.xml";
                            }
                            else if (RequestType == "2")
                            {
                                if (Convert.ToString(dtexcel.Rows[row]["F2"]).Trim() == "")
                                {
                                    vMExcel.Message = "Client Reference ID can't be blank.";
                                    vMExcel.Status = PStatus.Failed;
                                    break;
                                }

                                // Note Update - starttag
                                xml.Append(@"<?xml version=""1.0""?><NoteUpdate xmlns:xsd = ""http://www.w3.org/2001/XMLSchema"" xmlns:xsi = ""http://www.w3.org/2001/XMLSchema-instance"">");

                                #region Note master Data
                                xml.Append("<SystemSource>" + Convert.ToString(dtexcel.Rows[row]["F1"]).Trim() + "</SystemSource>");
                                xml.Append("<ClientReferenceID>" + Convert.ToString(dtexcel.Rows[row]["F2"]).Trim() + "</ClientReferenceID>");
                                xml.Append("<DateTimeStamp>" + DateTime.Now + "</DateTimeStamp>");
                                #endregion

                                #region Note Detail data
                                if (Convert.ToString(dtexcel.Rows[row]["F3"]).Trim() != "" ||
                                    Convert.ToString(dtexcel.Rows[row]["F5"]).Trim() != "")
                                {
                                    if (!StaticUtilities.IsSubstringValueExist(xml.ToString(), NotesTag[0])) xml.Append(NotesTag[0]);
                                    #region Note
                                    // Note - starttag
                                    xml.Append(NotesTag[2]);

                                    xml.Append("<NoteCreator>" + Convert.ToString(dtexcel.Rows[row]["F3"]).Trim() + "</NoteCreator>");
                                    xml.Append("<Note_GF>" + Convert.ToString(dtexcel.Rows[row]["F5"]).Trim() + "</Note_GF>");

                                    // Note - endtag
                                    xml.Append(NotesTag[3]);
                                    #endregion
                                }
                                if (StaticUtilities.IsSubstringValueExist(xml.ToString(), NotesTag[0])) xml.Append(NotesTag[1]);
                                #endregion

                                // Note Update - endtag
                                xml.Append("</NoteUpdate>");

                                // create the file
                                vMExcel.ConvertedXmlFileName = DateTime.Now.ToString("yyyyMMdd") + "-" + DateTime.Now.ToString("HHmmssfff") + "-" + new RandomGenerator().RandomGuid() + "-Note-Update.xml";
                            }
